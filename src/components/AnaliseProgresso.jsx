...
import { AlertTriangle, BarChart3, Bot, CarFront, Check, CheckCircle2, Code2, Cpu, FileUp, Gamepad2, Gauge, LineChart, Plus, Save, Trash2, Truck, Upload, X, Zap } from "lucide-react";
...
export default function AnaliseProgresso({ cursos = [], analises = [], isAdmin, onImportar, onExcluir, onCriarCurso }) {
  ...
  useEffect(() => {
    if (!cursoId && cursosVisiveis.length) setCursoId(cursosVisiveis[0].id);
    if (cursoId && !cursosVisiveis.some((curso) => curso.id === cursoId)) setCursoId(cursosVisiveis[0]?.id || "");
  }, [cursoId, cursosVisiveis]);
